...
registerRawTool(
  "screenshot",
  {
    ...
    inputSchema: {
      nodeId: z
        ...
        .optional()
        .describe(
          "ID of the node to screenshot. Uses current selection if omitted.",
        ),
      scale: z
        .number()
        .min(0.01)
        ...
        .optional()
        .describe(
          "Export scale multiplier (default: 1). Use 2 for retina-quality.",
        ),
      maxDimension: z
        .number()
        .min(64)
        ...
        .optional()
        .describe(
          "Maximum width or height in pixels (default: 1024, max: 2000). " +
  ...
  async (args) => {
    ...
    logger.info(
      `Screenshot raw PNG: ${rawBuffer.length} bytes from "${result.nodeName}"`,
    );
    ...
    const { width: outW, height: outH } = await sharp(optimized).metadata();
    const optimizedBase64 = optimized.toString("base64");
    logger.info(
      `Screenshot optimized PNG: ${outW}x${outH}, ${optimized.length} bytes (${Math.round((1 - optimized.length / rawBuffer.length) * 100)}% reduction)`,
    );
